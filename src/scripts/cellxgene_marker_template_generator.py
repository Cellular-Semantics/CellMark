...
def write_templates(mapping: dict, directory: str) -> None:
    """
    Write two ROBOT template TSV files for CellxGene markers based on the provided mapping.

    This function generates:
    1. `cellxgene_marker_annotations_template.tsv`: encodes CL–marker relationships with annotations
       such as marker score, cell ratio, tissue, species, source, and reference.
    2. `cellxgene_marker_template.tsv`: contains unique marker gene IRIs with their labels and superclass
       declaration for ontology inclusion.

    Each output file follows the ROBOT template format with two header rows.

    Args:
        mapping (dict): Nested dictionary of the form
            {taxon_uri: {uberon_uri: {cl_term: [marker_dict, ...]}}}
            where each marker_dict includes gene IRI, label, and annotations.
        directory (str): Path to the directory where the TSV files will be written.

    Returns:
        None
    """
    unique_marker_set = set()
    primary_template = os.path.join(directory, "cellxgene_marker_annotations_template.tsv")
    with open(primary_template, 'w', encoding='utf-8') as out:
        # template header
        out.write("ID\tMARKER\tMARKER_SCORE\tCELL_RATIO\tTISSUE\tSPECIES\tSOURCE\tREFERENCE\n")
        out.write("ID\tAI CLM:0009999\t>AT CLM:0009998^^xsd:float\t>AT CLM:0010002^^xsd:float\t>AI CLM:0009997\t>AI CLM:0009996\t>AI dcterms:source\t>AI dcterms:references\n")
            ...
            for uberon_uri, cl_map in tissues.items():
                for cl_term, markers in cl_map.items():
                    for m in markers:
                        ...
                        # write row
                        out.write(f"{cl_term}\t{gene}\t{score}\t{ratio}\t{uberon_uri}\t{taxon_uri}\thttps://cellxgene.cziscience.com/gene-expression\thttps://doi.org/10.1101/2023.10.30.563174\n")
